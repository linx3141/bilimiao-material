...
class UserFansPage(
    ...
    override fun Content() {
        val viewModel = diViewModel { UserFansPageViewModel(it) }
        val mid = id
        LaunchedEffect(mid) {
            viewModel.mid = mid
        }
        UserFansPageContent(viewModel = viewModel)
    }
}

private class UserFansPageViewModel(
    override val di: DI,
) : ViewModel(), DIAware {
    ...
    fun attention(
        ...
        try {
            ...
            val mode = if (item.isFollowing) 2 else 1
            val newAttribute = if (item.isFollowing) 0 else 2
            val res = BiliApiService.userRelationApi
...
@Composable
private fun UserFansPageContent(
    viewModel: UserFansPageViewModel,
) {
